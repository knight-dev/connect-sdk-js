...
### Added
- New `PackageType` union (`'Box' | 'Bag' | 'Envelope' | 'Tube' | 'Crate' | 'Pallet' | 'Irregular' | 'Other'`) and `PackageCondition` union (`'Good' | 'MinorDamage' | 'ModerateDamage' | 'SevereDamage' | 'Tampered' | 'WetDamaged' | 'Fragile'`).
- `Package` now exposes `packageType`, `condition`, `conditionNotes`, `sourceMarketplace`, `merchantName`, plus full dimensions: `lengthIn`, `widthIn`, `heightIn`, `dimensionalWeightLbs`, `billableWeightLbs`.
- `CreatePackageInput` accepts `shipperAddressCode`, `sourceMarketplace`, `merchantName`, `packageType`, `freightType` (alternative to `shipperId`).
- `UpdatePackageInput` accepts `sourceMarketplace`, `merchantName`, `packageType`, `freightType`, `condition`, `conditionNotes`.
- `PackageReceivedPayload` and `IntakeUnclaimedPayload` carry the same expanded field set so webhook consumers don't need a follow-up `GET /packages/{id}` for routing decisions.
- `PackageUpdatedPayload` adds `dimensionalWeightLbs`, `billableWeightLbs`, `packageType`, `condition`, `conditionNotes`, `sourceMarketplace`.
