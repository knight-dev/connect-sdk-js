...
const m = await client.manifests.create({
  type: 'Air',
  carrierName: 'FedEx',
  originCode: 'MIA',
  destinationCode: 'KIN',
  estimatedDeparture: new Date('2026-05-01T20:00:00Z'),
  estimatedArrival: new Date('2026-05-02T08:00:00Z'),
...
await client.manifests.setStatus(m.id, {
  status: 'Shipped',
  actualDeparture: new Date()
});

// 6+. Progress through customs / arrival / completion as events happen.
await client.manifests.setStatus(m.id, { status: 'AtCustoms' });
await client.manifests.setStatus(m.id, { status: 'Cleared' });
await client.manifests.setStatus(m.id, { status: 'Arrived' });
await client.manifests.setStatus(m.id, { status: 'Completed' });
```

## Open / close / reopen semantics

```ts
await client.manifests.close(m.id);              // IsOpen := false, status unchanged
await client.manifests.reopen(m.id);             // IsOpen := true
await client.manifests.reopen(m.id, true);       // Also auto-link currently-unassigned packages
```

`reopen()` on a Finalized manifest fails — the manifest's status is locked. If you need to add a straggler package to a finalized manifest, you have two options:
...
The detail endpoint embeds packages, but for manifests with hundreds of packages you'll want pagination:

```ts
const page = await client.packages.forManifest(m.id, { page: 1, pageSize: 50 });
console.log(`${page.pagination.totalCount} packages on manifest`);
```
...
```ts
await client.manifests.delete(m.id);  // 400 if Finalized, or if any package is attached
```
...
| Event | When |
|---|---|
| `manifest.created` | A new manifest was opened (via your SDK call, or by the warehouse UI) |
